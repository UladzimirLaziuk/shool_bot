...
from babyhelpergpt.chains import HelperConversationChain

GPT_MODEL = "gpt-3.5-turbo-0613"
# GPT_MODEL_16K = "gpt-3.5-turbo-16k-0613"


class BabyChatGPTAPI:
    USE_TOOLS = False
    ...
    def do(self, conversation_history: [str], human_input=None):
        if self.config_path == "":
            ...
            # USE_TOOLS = True
            if self.USE_TOOLS:
                helper_agent = HelperConversationChain.from_llm(
                    self.llm,
                    use_tools=True,
                ...
                helper_agent = HelperConversationChain.from_llm(self.llm, verbose=self.verbose)
            ...
            helper_agent = BabyChatGPT.from_llm(self.llm, verbose=self.verbose, **config)
        ...
        helper_agent.conversation_history = conversation_history

        if human_input is not None:
            helper_agent.human_step(human_input)
        ...
        reply = helper_agent.conversation_history[-1]

        if self.verbose:
            print("=" * 10)
            print(f"{helper_agent.helper_person_name}:{reply}")
        return reply.split(": ")
